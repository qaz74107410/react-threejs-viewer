...
const GLTFLoader = (() => {
    ...
    class GLTFLightsExtension {
        constructor({extensions}) {
            ...
            const extension = ( extensions && extensions[ EXTENSIONS.KHR_LIGHTS_PUNCTUAL ] ) || {};
    ...
    class GLTFMaterialsUnlitExtension {
        ...
        extendParams(materialParams, {pbrMetallicRoughness}, parser) {
            ...
            const metallicRoughness = pbrMetallicRoughness;
    ...
    class GLTFDracoMeshCompressionExtension {
        ...
        decodePrimitive({extensions, attributes}, parser) {

            const json = this.json;
            const dracoLoader = this.dracoLoader;
            const bufferViewIndex = extensions[ this.name ].bufferView;
            const gltfAttributeMap = extensions[ this.name ].attributes;
            ...
            for ( attributeName in attributes ) {
                ...
                if ( gltfAttributeMap[ attributeName ] !== undefined ) {
                    ...
                    const accessorDef = json.accessors[ attributes[ attributeName ] ];
    ...
    class GLTFTextureTransformExtension {
        ...
        extendTexture(texture, {offset, rotation, scale, texCoord}) {
    ...
    function GLTFMaterialsPbrSpecularGlossinessExtension() {
		...
		return {
			...
			extendParams(materialParams, {extensions}, parser) {

				const pbrSpecularGlossiness = extensions[ this.name ];
			...
			refreshUniforms({properties}, scene, camera, geometry, material, group) {
    ...
    function addUnknownExtensionsToUserData(knownExtensions, {userData}, {extensions}) {
		...
		for ( const name in extensions ) {
			...
			if ( knownExtensions[ name ] === undefined ) {
				...
				userData.gltfExtensions[ name ] = extensions[ name ];
	 ...
	 */
    function assignExtrasToUserData(object, {extras}) {
		...
		if ( extras !== undefined ) {
			...
			if ( typeof extras === 'object' ) {
				...
				console.warn( `GLTFLoader: Ignoring primitive type .extras, ${extras}` );
	 ...
	 */
    function updateMorphTargets(mesh, {weights, extras}) {
    ...
    function createPrimitiveKey({extensions, indices, attributes, mode}) {

		const dracoExtension = extensions && extensions[ EXTENSIONS.KHR_DRACO_MESH_COMPRESSION ];
		let geometryKey;
		...
		if ( dracoExtension ) {
			...
			geometryKey = `${indices}:${createAttributesKey( attributes )}:${mode}`;
    ...
    class GLTFParser {
         ...
         * @return {Promise}
         */
        assignTexture(materialParams, mapName, {index, extensions}) {

            const parser = this;

            return this.getDependency( 'texture', index ).then( texture => {
                ...
                if ( parser.extensions[ EXTENSIONS.KHR_TEXTURE_TRANSFORM ] ) {
                    ...
                    const transform = extensions !== undefined ? extensions[ EXTENSIONS.KHR_TEXTURE_TRANSFORM ] : undefined;
                ...
                if ( nodeDef.isBone === true ) {
                    ...
                    return parser.getDependency( 'mesh', nodeDef.mesh ).then( mesh => {
                        ...
                        if ( nodeDef.weights !== undefined ) {
                            ...
                            node.traverse( ({isMesh, morphTargetInfluences}) => {

                                if ( ! isMesh ) return;

                                for ( let i = 0, il = nodeDef.weights.length; i < il; i ++ ) {

                                    morphTargetInfluences[ i ] = nodeDef.weights[ i ];
	 ...
	 */
    GLTFParser.prototype.loadScene = (() => {
