...
SVGLoader.prototype = {
	...
	parse(text) {
		...
		*/
		...
		function isTransformRotated({elements}) {
			return elements[ 1 ] !== 0 || elements[ 3 ] !== 0;
		}

		function getTransformScaleX({elements}) {
			const te = elements;
			return Math.sqrt( te[ 0 ] * te[ 0 ] + te[ 1 ] * te[ 1 ] )
		}

		function getTransformScaleY({elements}) {
			const te = elements;
...
SVGLoader.pointsToStroke = (points, style, arcDivisions, minDistance) => {
...
SVGLoader.pointsToStrokeWithBuffers = (() => {
	...
	return (
        points,
        {strokeWidth, strokeLineJoin, strokeMiterLimit, strokeLineCap},
        arcDivisions,
        minDistance,
        vertices,
        normals,
		...
		for ( let iPoint = 1; iPoint < numPoints; iPoint ++ ) {
			...
			if ( nextPoint !== undefined ) {
				...
				const maxInnerDistance = tempV2_3.normalize();
				...
				if ( dot !== 0 ) {
					...
					switch ( strokeLineJoin ) {
						...
						default:
							...
							const miterFraction = ( strokeWidth2 * strokeMiterLimit ) / miterLength2;

							if ( miterFraction < 1 ) {

								// The join miter length exceeds the miter limit

								if ( strokeLineJoin !== 'miter-clip' ) {
		...
		function addVertex({x, y}, u, v) {

			if ( vertices ) {

				vertices[ currentCoordinate ] = x;
				vertices[ currentCoordinate + 1 ] = y;
		...
		function makeCircularSector( center, p1, p2, u, v ) {
			...
			for ( let i = 0, il = arcLengthDivisions - 1; i < il; i++ ) {
