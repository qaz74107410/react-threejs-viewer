...
import { MMDParser } from './libs/MMDParser';

const THREE = (function(THREEjs) {

	THREEjs.TGALoader = TGALoader;
	return THREEjs
...
const MMDLoader = ( function () {
	...
	MMDLoader.prototype = {
		...
		/**
		 * Parse .pmd buffer to skinmesh object.
		 *
		 * @author komsan
		 * 
		 * @param {string} buffer
		 ...
		 * @param {function} onLoad
		 * @param {function} onError
		 * @return {THREE.Skinmesh}
		 */

		parsePMD(buffer, path, onLoad, onError) {
			const parser = this._getParser();
			const builder = this.meshBuilder.setCrossOrigin( this.crossOrigin );
			const onProgress = undefined;
			const resourcePath = '';

			const mesh = builder.build( parser.parsePmd( buffer, true ), resourcePath, onProgress, onError );
		...
		/**
		 * Parse .pmx buffer to skinmesh object.
		 *
		 * @author komsan
		 * 
		 * @param {string} buffer
		...
		parsePMX(buffer, path, onLoad, onError) {
			const parser = this._getParser();
			const builder = this.meshBuilder.setCrossOrigin( this.crossOrigin );
			const onProgress = undefined;
			const resourcePath = '';

			const mesh = builder.build( parser.parsePmx( buffer, true ), resourcePath, onProgress, onError );
		...
		_getTGALoader: function () {
			...
			if ( this.tgaLoader === null ) {
				...
				if ( THREE.TGALoader === undefined ) {
					...
					throw new Error( 'MMDLoader: Import THREE.TGALoader' );
