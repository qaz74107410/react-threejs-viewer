...
Object.assign( VTKLoader.prototype, THREE.EventDispatcher.prototype, {
	...
	parse(data) {
			...
			function parseDataArray( ele, compressed ) {
				...
				if ( ele.attributes.format === 'binary' && compressed ) {
                    ...
                    for ( var i = 0; i < dataOffsets.length - 1; i ++ ) {
						...
						const inflate = new Zlib.Inflate( byteData.slice( dataOffsets[ i ], dataOffsets[ i + 1 ] ), { resize: true, verify: true } ); // eslint-disable-line no-undef
					...
					if ( ele.attributes.format === 'binary' && ! compressed ) {
						...
						if ( ele[ '#text' ] ) {
							...
							var content = ele[ '#text' ].split( /\s+/ ).filter( el => {
		...
		function getStringFile( data ) {
			...
			let stringFile = '';
			const charArray = new Uint8Array( data );
			let i = 0;
