...
import NURBSCurve from '../util/NURBSCurve';

const THREE = (function (THREEjs) {

	THREEjs.NURBSCurve = NURBSCurve;
	return THREEjs;
...
const FBXLoader = (() => {
	...
	FBXLoader.prototype = {
		...
		load(url, onLoad, onProgress, onError) {
			...
			const path = ( self.path === undefined ) ? THREE.LoaderUtils.extractUrlBase( url ) : self.path;
	...
	FBXTreeParser.prototype = {
		...
		parseImage({Content, RelativeFilename, Filename}) {

			const content = Content;
			const fileName = RelativeFilename || Filename;
		...
		loadTexture({id, FileName}, images) {
		...
		parseSkeleton({children}, deformerNodes) {

			const rawBones = [];

			children.forEach( ({ID}) => {

				const boneNode = deformerNodes[ ID ];
		...
		parseMorphTargets({children}, deformerNodes) {
			...
			for ( let i = 0; i < children.length; i ++ ) {
				...
				rawMorphTarget.geoID = connections.get( parseInt( child.ID ) ).children.filter( ({relationship}) => {
		...
		parseScene({skeletons}, geometryMap, materialMap) {

			sceneGraph = new THREE.Group();

			const modelMap = this.parseModels( skeletons, geometryMap, materialMap );
			...
			modelMap.forEach( model => {
				...
				parentConnections.forEach( ({ID}) => {

					const parent = modelMap.get( ID );
			...
			this.bindSkeleton( skeletons, geometryMap, modelMap );
		...
		buildSkeleton({parents}, skeletons, id, name) {
		...
		createCamera({children}) {
			...
			children.forEach( ({ID}) => {
				...
				const attr = fbxTree.Objects.NodeAttribute[ ID ];
		...
		createMesh({children}, geometryMap, materialMap) {
		...
		createCurve({children}, geometryMap) {

			const geometry = children.reduce( (geo, {ID}) => {

				if ( geometryMap.has( ID ) ) geo = geometryMap.get( ID );
		...
		getTransformData({userData}, modelNode) {
		...
		setLookAtProperties(model, modelNode) {
			...
			if ( 'LookAtProperty' in modelNode ) {
				...
				children.forEach( ({relationship, ID}) => {

					if ( relationship === 'LookAtProperty' ) {

						const lookAtTarget = fbxTree.Objects.Model[ ID ];
				...
				for ( const nodeID in BindPoseNode ) {
					...
					if ( BindPoseNode[ nodeID ].attrType === 'BindPose' ) {
						...
						if ( Array.isArray( poseNodes ) ) {
							...
							poseNodes.forEach( ({Node, Matrix}) => {

								bindMatrices[ Node ] = new THREE.Matrix4().fromArray( Matrix.a );
	...
	GeometryParser.prototype = {
		...
		parseMeshGeometry({parents, children}, geoNode, deformers) {

			const skeletons = deformers.skeletons;
			const morphTargets = deformers.morphTargets;

			const modelNodes = parents.map( ({ID}) => {

				return fbxTree.Objects.Model[ ID ];
			...
			const skeleton = children.reduce( (skeleton, {ID}) => {

				if ( skeletons[ ID ] !== undefined ) skeleton = skeletons[ ID ];

				return skeleton;

			}, null );

			const morphTarget = children.reduce( (morphTarget, {ID}) => {

				if ( morphTargets[ ID ] !== undefined ) morphTarget = morphTargets[ ID ];
		...
		parseGeoNode(geoNode, skeleton) {
			...
			if ( skeleton !== null ) {
				...
				skeleton.rawBones.forEach( ({indices, weights}, i) => {
			...
			geoInfo.vertexIndices.forEach( (vertexIndex, polygonVertexIndex) => {
				...
				if ( geoInfo.skeleton ) {
					...
					if ( geoInfo.weightTable[ vertexIndex ] !== undefined ) {
						...
						geoInfo.weightTable[ vertexIndex ].forEach( ({weight, id}) => {

							weights.push( weight );
							weightIndices.push( id );
		...
		addMorphTargets(parentGeo, parentGeoNode, morphTarget, preTransform) {
			...
			const self = this;
			morphTarget.rawTargets.forEach( ({geoID, name}) => {

				const morphGeoNode = fbxTree.Objects.Geometry[ geoID ];

				if ( morphGeoNode !== undefined ) {

					self.genMorphGeometry( parentGeo, parentGeoNode, morphGeoNode, preTransform, name );
		...
		parseUVs({MappingInformationType, ReferenceInformationType, UV, UVIndex}) {

			const mappingType = MappingInformationType;
			const referenceType = ReferenceInformationType;
			const buffer = UV.a;
			let indexBuffer = [];
			if ( referenceType === 'IndexToDirect' ) {

				indexBuffer = UVIndex.a;
		...
		parseVertexColors({MappingInformationType, ReferenceInformationType, Colors, ColorIndex}) {

			const mappingType = MappingInformationType;
			const referenceType = ReferenceInformationType;
			const buffer = Colors.a;
			let indexBuffer = [];
			if ( referenceType === 'IndexToDirect' ) {

				indexBuffer = ColorIndex.a;
		...
		parseMaterialIndices({MappingInformationType, ReferenceInformationType, Materials}) {
			...
			const materialIndexBuffer = Materials.a;
	...
	AnimationParser.prototype = {
			...
			for ( const nodeID in rawLayers ) {
				...
				if ( connection !== undefined ) {
					...
					children.forEach( ({ID}, i) => {
						...
						if ( curveNodesMap.has( ID ) ) {

							const curveNode = curveNodesMap.get( ID );
							...
							if ( curveNode.curves.x !== undefined || curveNode.curves.y !== undefined || curveNode.curves.z !== undefined ) {
								...
								if ( layerCurveNodes[ i ] === undefined ) {
									...
									var modelID = connections.get( ID ).parents.filter( ({relationship}) => {
									...
									if ( modelID !== undefined ) {
										...
										sceneGraph.traverse( ({ID, matrix, userData}) => {

											if ( ID === rawModel.id ) {

												node.transform = matrix;

												if ( userData.transformData ) node.eulerOrder = userData.transformData.eulerOrder;
								...
								if ( layerCurveNodes[ i ] === undefined ) {
									...
									const deformerID = connections.get( ID ).parents.filter( ({relationship}) => {
		...
		generateMorphTrack({DeformPercent, modelName, morphName}) {

			const curves = DeformPercent.curves.morph;
			...
			const morphNum = sceneGraph.getObjectByName( modelName ).morphTargetDictionary[ morphName ];

			return new THREE.NumberKeyframeTrack( `${modelName}.morphTargetInfluences[${morphNum}]`, curves.times, values );

		},

		// For all animated objects, times are defined separately for each axis
		// Here we'll combine the times into one sorted array without duplicates
		getTimesForAllAxes({x, y, z}) {

			let times = [];

			// first join together the times for each axis, if defined
			if ( x !== undefined ) times = times.concat( x.times );
			if ( y !== undefined ) times = times.concat( y.times );
			if ( z !== undefined ) times = times.concat( z.times );
		...
		getKeyframeTrackValues(times, {x, y, z}, initialValue) {
			...
			times.forEach( time => {
				...
				if ( x ) xIndex = x.times.indexOf( time );
				if ( y ) yIndex = y.times.indexOf( time );
				if ( z ) zIndex = z.times.indexOf( time );

				// if there is an x value defined for this frame, use that
				if ( xIndex !== - 1 ) {

					const xValue = x.values[ xIndex ];
					values.push( xValue );
					prevValue[ 0 ] = xValue;
	...
	function generateTransform( transformData ) {
		...
		let lParentGX = new THREE.Matrix4();
		...
		if ( inheritType === 0 ) {
			...
			const lParentLSM = new THREE.Matrix4().copy( lScalingM );
	...
	function getEulerOrder(order = 0) {
        const enums = [
			'ZYX', // -> XYZ extrinsic  
			'YZX', // -> XZY extrinsic
			'XZY', // -> YZX extrinsic
